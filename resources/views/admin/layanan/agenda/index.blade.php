<x-admin-layout>
    ...
    <x-admin.heading name="Agenda">
        <a href="{{ route('agenda.create') }}" class="modern-btn modern-btn-primary">
                ...
                <table class="modern-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="fw-bold">{{ convertDatetileLocToDate($value->jadwal) }}</div>
                                    <small class="text-muted">{{ convertDatetileLocToTime($value->jadwal)}}</small>
                                </td>
                                <td>
                                    @if($value->jadwal_akhir)
                                        <div class="fw-bold">{{ convertDatetileLocToDate($value->jadwal_akhir) }}</div>
                                        <small class="text-muted">{{convertDatetileLocToTime($value->jadwal_akhir)}}</small>
                                    @else
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        @if($value->is_shown)
                                        <a href="{{ route('layanan-agenda.show', ['slug'=>$value->slug]) }}" 
                                           class="modern-btn modern-btn-success modern-btn-sm" target="_blank">
                                            <i class="fa fa-eye me-1"></i> Lihat
                                        </a>
                                        @endif
                                        <a href="{{ route('agenda.edit', ['agenda'=>$value->id]) }}" 
                                        ...
                                        <button type="button" class="modern-btn modern-btn-danger modern-btn-sm show_confirm" 
                                                data-bs-toggle="modal" data-bs-target="#deleteAgenda" 
                                                data-id-agenda="{{ $value->id }}" data-judul-agenda="{{ $value->judul }}">
    ...
    <x-slot name="modals">
        <div class="modal fade modern-modal" id="deleteAgenda" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    ...
                    <form method="POST" action="#">
                        ...
                        <div class="modal-body">
                            <div class="modern-alert modern-alert-warning">
                                ...
                                Apakah Anda yakin ingin menghapus agenda yang berjudul <strong class="text-primary"></strong>?
                            </div>
                        </div>
