        ...
        <div class="row mt-3">
            <div class="col-md-12">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h3 class="mb-0"><i class="fa fa-clipboard me-2"></i>Detail Proposal Penelitian</h3>
                    <a href="{{ route('penelitian-adm.index') }}" class="modern-btn modern-btn-secondary">
                ...
                @if ($timeline && $timeline->admin_decision_start_date && $timeline->admin_decision_end_date)
                    ...
                    @if (!$matchPeriod)
                        <div class="modern-alert modern-alert-warning mb-3">
                            <i class="fa fa-lock me-2"></i>Proposal ini berada pada periode {{ $proposalYear ?? '-' }},
                            sedangkan periode penyetujuan aktif adalah {{ $timeline->period }}. Penyetujuan tidak dapat
                            dilakukan.
                        </div>
                    @elseif ($currentDate < $timeline->admin_decision_start_date)
                        <div class="modern-alert modern-alert-warning mb-3">
                            <i class="fa fa-clock me-2"></i>Periode penyetujuan admin akan dimulai pada
                            <strong>{{ $timeline->admin_decision_start_date->format('d F Y H:i') }}</strong>.
                        </div>
                    @elseif ($currentDate > $timeline->admin_decision_end_date)
                        <div class="modern-alert modern-alert-danger mb-3">
                            <i class="fa fa-times-circle me-2"></i>Periode penyetujuan admin telah berakhir pada
                            <strong>{{ $timeline->admin_decision_end_date->format('d F Y H:i') }}</strong>. Anda tidak dapat
                            melakukan perubahan keputusan.
                        </div>
                    @else
                        <div class="modern-alert modern-alert-info mb-3">
                            <i class="fa fa-info-circle me-2"></i>Periode penyetujuan admin sedang berlangsung. Akan berakhir
                            pada <strong>{{ $timeline->admin_decision_end_date->format('d F Y H:i') }}</strong>.
                        </div>
                    @endif
                @endif
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                                ...
                                                @foreach($assignedReviewers as $rev)
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <form
                                                                action="{{ route('penelitian-adm.remove-reviewer', ['id' => $proposal->id, 'reviewerId' => $rev->id]) }}"
                                                                method="POST"
                                                                onsubmit="return confirm('Apakah Anda yakin ingin membatalkan penugasan reviewer ini?');"
                                ...
                                @else
                                    <div class="alert alert-warning">
                                        <i class="fa fa-exclamation-triangle me-1"></i> Belum ada reviewer yang ditugaskan
                                        untuk proposal ini.
                                    </div>
                                @endif
                            </div>
                            <div class="col-md-4">
                                ...
                                <div class="p-3 border rounded bg-light">
                                    <form action="{{ route('penelitian-adm.assign-reviewer', $proposal->id) }}"
                                        method="POST">
                                        @csrf
                                        <div class="mb-3">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="d-flex flex-column p-3 border rounded-3 h-100">
                                    ...
                                    <span class="text-muted text-uppercase small fw-semibold mt-3">Luaran
                                        Tambahan</span>
                            ...
                            <div class="col-md-6">
                                <div class="d-flex flex-column p-3 border rounded-3 h-100">
                                    ...
                                    <span class="text-muted text-uppercase small fw-semibold mt-3">Biaya
                                        Diusulkan</span>
                                    <span class="fw-bold">Rp
                                        {{ number_format($proposal->biaya_diusulkan ?? 0, 0, ',', '.') }}</span>
                            ...
                            <div class="col-12">
                                <div class="p-3 border rounded-3">
                                    <span class="text-muted text-uppercase small fw-semibold d-block mb-2">Ringkasan
                                        Proposal</span>
                                    <p class="mb-0 text-muted">
                                        {{ $proposal->ringkasan_proposal ?? 'Ringkasan belum tersedia.' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modern-card mb-4 fade-in-up">
                    <div class="modern-card-body">
                        ...
                        <iframe src="{{ $fileUrl }}"
                            style="width:100%; height:700px; border: 1px solid #ddd; border-radius: 8px;"></iframe>
                    </div>
                </div>
                                    ...
                                    <div class="modern-card">
                                        <div class="modern-card-body">
                                            ...
                                            <p class="text-muted mb-2">
                                                ...
                                                {{ $review->reviewer->name ?? $review->reviewer_name ?? 'Tidak diketahui' }}
                                            </p>
                                                    ...
                                                    <table class="modern-table">
                                                        ...
                                                        <tbody>
                                                            @if($review->reviewKriteria && $review->reviewKriteria->count() > 0)
                                                                ...
                                                                @foreach($review->reviewKriteria->filter(function ($rk) {
                                                                    return $rk->formPenilaianReview !== null; })->sortBy('formPenilaianReview.urutan') as $reviewKriteria)
                                                                    <tr>
                                                                        <td>{!! nl2br(e($reviewKriteria->formPenilaianReview->kriteria_penilaian ?? '-')) !!}
                                                                        </td>
                                                                        <td>{{ $reviewKriteria->formPenilaianReview->bobot == floor($reviewKriteria->formPenilaianReview->bobot) ? number_format($reviewKriteria->formPenilaianReview->bobot, 0) : number_format($reviewKriteria->formPenilaianReview->bobot, 2) }}%
                                                                        </td>
                                                            ...
                                                            @else
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>20%</td>
                                                                    <td>{{ $review->skor_1 ?? '-' }}</td>
                                                                    <td>{{ $review->skor_1 ? ($review->skor_1 * 20) : '-' }}
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    ...
                                                                    <td>20%</td>
                                                                    <td>{{ $review->skor_2 ?? '-' }}</td>
                                                                    <td>{{ $review->skor_2 ? ($review->skor_2 * 20) : '-' }}
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    ...
                                                                    <td>20%</td>
                                                                    <td>{{ $review->skor_3 ?? '-' }}</td>
                                                                    <td>{{ $review->skor_3 ? ($review->skor_3 * 20) : '-' }}
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    ...
                                                                    <td>10%</td>
                                                                    <td>{{ $review->skor_4 ?? '-' }}</td>
                                                                    <td>{{ $review->skor_4 ? ($review->skor_4 * 10) : '-' }}
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    ...
                                                                    <td>30%</td>
                                                                    <td>{{ $review->skor_5 ?? '-' }}</td>
                                                                    <td>{{ $review->skor_5 ? ($review->skor_5 * 30) : '-' }}
                                                                    </td>
                                                                </tr>
                                                            @endif
                                                        </tbody>
                ...
                <div class="modern-card mb-4 fade-in-up">
                    <div class="modern-card-body">
                        ...
                        @if($proposal->admin_status)
                            <div
                                class="modern-alert {{ $proposal->admin_status === 'approved' ? 'modern-alert-success' : 'modern-alert-danger' }} mb-3">
                                <i
                                    class="fa fa-{{ $proposal->admin_status === 'approved' ? 'check' : 'times' }}-circle me-2"></i>
                                Proposal telah
                                <strong>{{ $proposal->admin_status === 'approved' ? 'disetujui' : 'ditolak' }}</strong>
                        ...
                        <form action="{{ route('penelitian-adm.approve-reject', $proposal->id) }}" method="POST">
                            @csrf

                            <div class="row mb-3">
                                <div class="col-md-12">
                                    <label class="modern-form-label"><i class="fa fa-check-circle me-2"></i>Status
                                        Keputusan</label>
                                    <div class="d-flex gap-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="admin_status"
                                                id="approved" value="approved" {{ $proposal->admin_status === 'approved' ? 'checked' : '' }} @if(!$canMakeDecision) disabled @endif>
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="admin_status"
                                                id="rejected" value="rejected" {{ $proposal->admin_status === 'rejected' ? 'checked' : '' }} @if(!$canMakeDecision) disabled @else required @endif>
                            ...
                            <div class="mb-3">
                                <label for="admin_comment" class="modern-form-label"><i
                                        class="fa fa-comment-dots me-2"></i>Komentar Admin <span
                                        class="text-danger">*</span></label>
                                <textarea id="admin_comment" name="admin_comment" class="modern-form-textarea" rows="4"
                                    placeholder="Masukkan komentar atau catatan untuk proposal ini..."
                                    @if(!$canMakeDecision) disabled @else required
                                    @endif>{{ $proposal->admin_comment ?? '' }}</textarea>
                                <small class="text-muted">Komentar ini akan ditampilkan kepada dosen pengusul. <span
                                        class="text-danger">Wajib diisi.</span></small>
                            </div>
                            ...
                            <div class="mb-3">
                                <label class="modern-form-label"><i class="fa fa-money-bill-wave me-2"></i>Biaya yang
                                    Disetujui <span class="text-danger">*</span></label>
                                <div class="mb-2">
                                    <div class="form-check mb-2">
                                        <input class="form-check-input biaya-option" type="radio" name="biaya_source"
                                            id="biaya_diusulkan" value="diusulkan" {{ old('biaya_source', $biayaDisetujui == $biayaDiusulkan ? 'diusulkan' : '') == 'diusulkan' ? 'checked' : '' }} @if(!$canMakeDecision) disabled @else required @endif>
                                        <label class="form-check-label" for="biaya_diusulkan">
                                            Biaya Diusulkan Dosen: <strong>Rp
                                                {{ number_format($biayaDiusulkan, 0, ',', '.') }}</strong>
                                        </label>
                                    </div>
                                    @if($biayaReviewer1 > 0)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input biaya-option" type="radio" name="biaya_source"
                                                id="biaya_reviewer1" value="reviewer1" {{ old('biaya_source', $biayaDisetujui == $biayaReviewer1 ? 'reviewer1' : '') == 'reviewer1' ? 'checked' : '' }} @if(!$canMakeDecision) disabled @endif>
                                            <label class="form-check-label" for="biaya_reviewer1">
                                                Biaya Reviewer 1: <strong>Rp
                                                    {{ number_format($biayaReviewer1, 0, ',', '.') }}</strong>
                                            </label>
                                        </div>
                                    @endif
                                    @if($biayaReviewer2 > 0)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input biaya-option" type="radio" name="biaya_source"
                                                id="biaya_reviewer2" value="reviewer2" {{ old('biaya_source', $biayaDisetujui == $biayaReviewer2 ? 'reviewer2' : '') == 'reviewer2' ? 'checked' : '' }} @if(!$canMakeDecision) disabled @endif>
                                            <label class="form-check-label" for="biaya_reviewer2">
                                                Biaya Reviewer 2: <strong>Rp
                                                    {{ number_format($biayaReviewer2, 0, ',', '.') }}</strong>
                                            </label>
                                        </div>
                                    @endif
                                    <div class="form-check mb-2">
                                        <input class="form-check-input biaya-option" type="radio" name="biaya_source"
                                            id="biaya_custom" value="custom" {{ old('biaya_source', ($biayaDisetujui && $biayaDisetujui != $biayaDiusulkan && $biayaDisetujui != $biayaReviewer1 && $biayaDisetujui != $biayaReviewer2) ? 'custom' : '') == 'custom' ? 'checked' : '' }} @if(!$canMakeDecision) disabled @endif>
                                        <label class="form-check-label" for="biaya_custom">
                                            Input Sendiri
                                        </label>
                                    </div>
                                </div>
                                <div class="mb-2" id="custom_biaya_container"
                                    style="display: {{ old('biaya_source', ($biayaDisetujui && $biayaDisetujui != $biayaDiusulkan && $biayaDisetujui != $biayaReviewer1 && $biayaDisetujui != $biayaReviewer2) ? 'custom' : '') == 'custom' ? 'block' : 'none' }};">
                                    <label for="biaya_custom_input" class="modern-form-label">Masukkan Biaya (Rp) <span
                                            class="text-danger">*</span></label>
                                    <input type="text" id="biaya_custom_input" name="biaya_custom"
                                        class="modern-form-input" inputmode="numeric"
                                ...
                                <input type="hidden" id="biaya_disetujui" name="biaya_disetujui"
                                    value="{{ old('biaya_disetujui', $biayaDisetujui) }}">
                                <small class="text-muted d-block"><span class="text-danger">Wajib dipilih.</span> Biaya
                                    yang dipilih akan menjadi biaya final yang disetujui untuk proposal ini.</small>
                            </div>
